...
function getWhere(conditions, rawConditions, index = 0) {
  ...
  _.forEach(conditions, (v, k) => {
    ...
    if (_.isArray(v)) {
      ...
      paramIndex += 1;
      const op = v.ignoreCase ? '~*' : '~';
      arr.push(`${convertColumnName(k)} ${op}  $${paramIndex}`);
      params.push(v.source);
    } else {
      paramIndex += 1;
...
class SQL {
  ...
  /**
   * 设置查询的options(包括limit offset groupBy orderBy等)
   *
   * @param {any} options
   * @memberof SQL
   */
    ...
    _.forEach(options, (v, k) => {
      switch (k) {
        ...
        case 'limit':
          ...
          break;
        case 'order': {
          const orders = _.isArray(v) ? v : [v];
          this.addOrderBy(...orders);
          break;
        }
        case 'group': {
          const groups = _.isArray(v) ? v : [v];
          this.addGroupBy(...groups);
          break;
        }
        case 'having': {
          const havings = _.isArray(v) ? v : [v];
  ...
  getInsert() {
    ...
    if (fields && fields.length) {
      ...
      if (_.includes(fields, '*')) {
        returnFields = _.keys(schema);
      }
      const fieldsStr = _.map(returnFields, convertColumnName).join(',');
  ...
  getIndex() {
    ...
    const indexName = `idx_${indexes.join('_')}`;
    const params = _.map(indexes, convertColumnName).join(', ');
    return [
      `CREATE INDEX CONCURRENTLY ${indexName} ON ${table}(${params})`,
    ];
  }
  /**
   * promise thenable
