...
class Table {
  constructor(client, table, schema, constraint) {
    if (!client || !table) {
  ...
  get sql() {
    ...
    if (!table || !client || !schema) {
      throw new Error('table client and schme can not be null');
    }
  ...
  insert(data, fields) {
    ...
    sql.then = (resolve, reject) => originalThen((result) => {
      if (sql.rawMode) {
